...
public class OperacionesNifNuevoImpl implements IOperacionesNif {

  public OperacionesNifNuevoImpl(){
  }
...
//    args[1]="generar";
    ...
    if (args.length!=2) {
      System.out.println("uso: java OperacionesNifNuevoImpl nif_nie generar|validar");
      System.exit(-1);
    }
    Salida salida = new Salida();
    OperacionesNifNuevoImpl nif = new OperacionesNifNuevoImpl();
    ...
    if ((dni > 0) && (dni <= 99999999)) {
      ...
      if (gasai.util.Misc.esVacio(letra)) {
        ...
        if (letra.equals("X")) {
          ...
          sdni = letra +
              gasai.util.Misc.rellenarIzq(dniAux, "0", 7) + l;
        }else{
          //si es Y o Z hay que coger el dni con el que hemos calculado y quitarle el 1 o el 2 correspondiente a Y y Z
          sdni = letra +
              gasai.util.Misc.rellenarIzq(dniAux.substring(1,dniAux.length()), "0", 7) + l;
        }
      }
...
*/
  ...
  public Salida valida(String elNif) {
    ...
    try {
      ...
      //comprobamos el número de caracteres, que puede ser 10,9 y mayor que 2 y menor que 9
      //1R es un nif válido
...
//          }
          ...
          if (longitud > 9){
            ...
            nifValidar = elNif.substring(1,longitud);
            //Si el cuerpo es menos de 8 digitos 7777777L se rellena por la izquierda
            if (nifValidar.length() < 8){
              nifValidar = gasai.util.Misc.rellenarIzq(nifValidar,"0",8);
            }
            //si el NIE empieza por Y hay que sustituir Y por 1 quedando 17777777L
            if (elNif.charAt(0) == 'Y') {
              //Es el nie de un extranjero
              nifValidar = "1" + nifValidar;
            }
            //si el NIE empieza por Y hay que sustituir Y por 2 quedando 27777777L
            if (elNif.charAt(0) == 'Z') {
      ...
      // Comprobar último carácter sea letra, si no es letra darlo por bueno
      if (!error){
      ...
      if (!error){
        salida.setOk();
        if (elNif.charAt(0) == 'X' || elNif.charAt(0) == 'Y' || elNif.charAt(0) == 'Z') {
          valorDevuelto += gasai.util.Misc.rellenarIzq(nifValidar.substring(1,nifValidar.length()), "0", 8);
        }else{
          valorDevuelto += nifValidar;
        }
        salida.setValor(valorDevuelto);
  ...
  public String formateoNie (String agente,String nie){
	    ...
	    if (!Misc.esVacio(agente) && sns.config.Inicializacion.vAGENTES_NIE.indexOf(agente)==-1 && nie.charAt(0)=='X'
	    	&& nie.length()==9){
